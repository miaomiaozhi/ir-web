...
for index in ['/tzgg.htm', '/xqdt/xqxw.htm']:
    ...
    nav_url = re.compile(r'<a href="(?P<url>.*?)">',
                         re.S).finditer(nav_html)
    for i, nav in enumerate(nav_url):
        # 找到当前页面的所有文章table
        table = browser.find_element(
            'xpath', '/html/body/div[4]/div/div[2]/ul')
        table_html = table.get_attribute('innerHTML')
        if index == '/tzgg.htm':
            article_url = re.compile(
                r'<a href="(?P<url>.*?)" class="wl">').finditer(table_html)
        elif index == '/xqdt/xqxw.htm':
            article_url = re.compile(
                r'<a href="(?P<url>.*?)" class="flex" title=".*?"').finditer(table_html)
        else:
            article_url = re.compile(
                r'<a href="(?P<url>.*?)" class="wl" title=".*?" target="_blank"').finditer(table_html)
        ...
        for j, article in enumerate(article_url):
            ...
            file_path = os.path.join(PATH, "pages", f"page_{cnt}.txt")
            cnt += 1
...
department_element = browser.find_element(
    'xpath', '/html/body/table/tbody/tr[2]/td/table/tbody/tr[3]/td/table/tbody/tr[2]/td/table/tbody/tr/td[12]/select[2]')
